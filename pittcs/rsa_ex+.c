...
int get_file_lines(FILE *fp) {
    ...
    /*
    while(1) {
        if (fgets(line, MAX, fp) == NULL) break; 
        lines++;
        //printf("%3d: %s", i, line);
    }
    ...
    */
    /*
    while (fscanf(fp, "%c%d", &data.let, &data.num) == 2) {
        if (data.let == '\n') {
            lines++;
        }
    }
...
void get_csvfile_contents(FILE *file, int arr[29][8]) {
    ...
    //while (fscanf(file, " %d,", &records[count].letter, &records[count].number) == 2) {
    //    count++;
    //}
...
int main(int datac, char *datav[]) {
    ...
    */
    ...
    //fprintf(f2, "%lld", mod_pow(ciph1[0][0],a_ciph1,n_ciph1)); // STEP 3) computing and storing val of ciph^a mod n in file
    //fprintf(f2, "\n");  // go to next line
    k = get_file_lines(f2);
    printf("f2 lines = %d\n", k);
    ...
    /* declare and allocate decipher vals array
    int** deciphs; 
    deciphs = (int**)malloc(get_file_lines(f2) * sizeof *deciphs);
    for (i = 0; i < get_file_lines(f2); i++)
    {
    deciphs[i] = (int*)malloc((sizeof(ciph1[29])/sizeof(*ciph1[29])) * sizeof *deciphs[i]);
    }
    */
