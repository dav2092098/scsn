...
must compile with: gcc rsa_ex.c -lm
*/
#include <time.h>
#include <stdio.h>
#include <assert.h>
...
int get_file_contents(FILE *file) {
    int i = 0;
    int count = 0;
    while(!feof(file)) {
      if (fscanf (file, "%d", &i)) {
...
factorize2 (n) {
	...
	for (i = 2; i <= sqrt(n); i++) { // starting from 2 until sqrt(n), once i > sqrt(n) all the prime factors have been seen (since if n has an integer root, all subsequent factors must divisible by that root (since n can be represented by additions of only that root, i.e. the root is a foundational unit of the number))
	...
	rewind(f);
	printf("There are %d prime factors:. \n", ceil(get_file_contents(f) / 2)); // will include new line chars so divide by 2 and round up
	while(!feof(f)) {
      fscanf (f, "%d", &j);
      printf("%d", j);
    }
